...
package object api extends MatrixRequests with MatrixParser {
  ...
  def send[T](request: Request)(implicit decoder: Decoder[T]): ZIO[MatrixClient, MatrixError, T] =
    for {
      response <- ZIO.serviceWithZIO[MatrixClient](_.send(request))
      decoded  <- as(response)(decoder)
    } yield decoded

  def sendWithAuth[T](action: Request)(implicit decoder: Decoder[T]): ZIO[AuthMatrixEnv, MatrixError, T] =
    for {
